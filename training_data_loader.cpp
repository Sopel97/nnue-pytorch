...
struct HalfKA_PC20Factorized {
    // Factorized features
    static constexpr int PIECE_INPUTS = HalfKA_PC20::NUM_SQ * HalfKA_PC20::NUM_PT;
    static constexpr int INPUTS = HalfKA_PC20::INPUTS + PIECE_INPUTS;

    static constexpr int MAX_PIECE_FEATURES = 32;
    static constexpr int MAX_ACTIVE_FEATURES = HalfKA_PC20::MAX_ACTIVE_FEATURES + MAX_PIECE_FEATURES;

    static void fill_features_sparse(int i, const TrainingDataEntry& e, int* features, float* values, int& counter, Color color)
    {
        auto counter_before = counter;
        int offset = HalfKA_PC20::fill_features_sparse(i, e, features, values, counter, color);
        auto& pos = e.pos;
        auto pieces = pos.piecesBB();
        ...
        for(Square sq : pieces)
        {
            auto p = pos.pieceAt(sq);
            auto p_idx = static_cast<int>(p.type()) * 2 + (p.color() != color);
            features_unordered[j++] = offset + (p_idx * HalfKA_PC20::NUM_SQ) + static_cast<int>(orient_flip(color, sq));
        }
        std::sort(features_unordered, features_unordered + j);
        for (int k = 0; k < j; ++k)
        {
...
extern "C" {
    ...
    )
    {
        ...
        {
            return new SparseBatch(FeatureSet<HalfKA_PC20>{}, entries);
        }
        else if (feature_set == "HalfKA_PC20^")
        {
    ...
    EXPORT Stream<SparseBatch>* CDECL create_sparse_batch_stream(const char* feature_set_c, int concurrency, const char* filename, int batch_size, bool cyclic, bool filtered, int random_fen_skipping)
    {
        ...
        {
            return new FeaturedBatchStream<FeatureSet<HalfKA_PC20>, SparseBatch>(concurrency, filename, batch_size, cyclic, skipPredicate);
        }
        else if (feature_set == "HalfKA_PC20^")
        {
