...
class Features(FeatureBlock):
  def __init__(self):
    super(Features, self).__init__('HalfKAv3', 0x1f234cb8, OrderedDict([('HalfKAv3', NUM_PLANES_REAL * NUM_SQ)]))

  def get_active_features(self, board: chess.Board):
    def piece_features(turn):
      indices = torch.zeros(NUM_PLANES_REAL * NUM_SQ)
      for sq, p in board.piece_map().items():
        indices[halfka_idx(turn, board.king(turn), sq, p)] = 1.0
      return indices
...
class FactorizedFeatures(FeatureBlock):
  def __init__(self):
    super(FactorizedFeatures, self).__init__('HalfKAv3^', 0x1f234cb8, OrderedDict([('HalfKAv3', NUM_PLANES_REAL * NUM_SQ), ('A', NUM_PLANES_VIRTUAL)]))
