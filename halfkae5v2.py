...
NUM_INPUTS = 5 * NUM_PLANES_REAL * NUM_SQ
  ...
  for ksq in range(64):
    for s in range(64):
      for pt, val in piece_values.items():
        ...
        for i in range(5):
          values[idxw + NUM_INPUTS // 5 * i] = val
          values[idxb + NUM_INPUTS // 5 * i] = -val

  return values

class Features(FeatureBlock):
  def __init__(self):
    super(Features, self).__init__('HalfKAE5v2', 0x5F1123B8, OrderedDict([('HalfKAE5v2', NUM_INPUTS)]))
...
class FactorizedFeatures(FeatureBlock):
  def __init__(self):
    super(FactorizedFeatures, self).__init__('HalfKAE5v2^', 0x5F1123B8, OrderedDict([('HalfKAE5v2', NUM_INPUTS), ('A', NUM_PLANES_VIRTUAL)]))
  ...
  def get_feature_factors(self, idx):
    ...
    k_idx = (idx % (NUM_INPUTS // 5)) // NUM_PLANES_REAL
