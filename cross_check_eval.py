...
def eval_model(model, item):
    us, them, white, black, imbalance_x, outcome, score, psqtindex, lsindex = item
    us = us.unsqueeze(dim=0)
    them = them.unsqueeze(dim=0)
    white = white.unsqueeze(dim=0)
    black = black.unsqueeze(dim=0)
    imbalance_x = imbalance_x.unsqueeze(dim=0)

    eval = model.forward(us, them, white, black, imbalance_x, psqtindex, lsindex).item() * 600.0
    if them[0] > 0.5:
        return -eval
    else:
        return eval
