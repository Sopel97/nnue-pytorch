...
def convert_ckpt(root_dir):
    ...
    for ckpt in ckpts:
        ...
        if not os.path.exists(nnue_file_name):
            command = "{} serialize.py {} {} \"--features=HalfKAv2^\"".format(sys.executable, ckpt, nnue_file_name)
...
def run_match(best, root_dir, c_chess_exe, concurrency, book_file_name, stockfish_base, stockfish_test):
    ...
    command = "{} -each tc=1000+1 nodes=20000 option.Hash=8 option.Threads=1 -gauntlet -games 200 -rounds 1 -concurrency {}".format(
